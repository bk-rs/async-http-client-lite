/*
cargo run -p async-http-client-lite-demo-async-net --bin ws echo.websocket.org 80 /
*/
...
use async_http_client_lite::{AsyncNetTcpConnector, Connector};
...
async fn run() -> io::Result<()> {
    ...
    println!("ws {} {} {}", domain, port, uri);

    //
    let addr = format!("{}:{}", domain, port);

    //
    let client = AsyncNetTcpConnector::connect(addr, None, None).await?;
